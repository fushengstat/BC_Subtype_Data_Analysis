...
setwd('/data/shengf2/simu_mtop/time/')
...
# time_mtop <- time_topo <- rep(NA,n.sizes)
#sizes <- c(5000,25000,50000,100000)


theta_test <- c(0,0,0,0,0,0,0)
...
for(s_size in 1:length(sizes)){
  ...
  y <- y.pheno.mis
      ...
      time_mtop[s_size] = mtop_run_time
      ...
      time_topo[s_size] = topo_run_time
}
...
save(result,file=paste0("./result_six/time_",i1,".Rdata"))
